...
var thumbplaceholderimg = 'nail_';
var thumbHolder = 'images/thumbplaceholder.gif';
function getFilesInFolder(folder) {
	...
	$.ajax({
		...
		$.each(data, 
			function(i, item) {
				$("#filesInFolder").append("<div class='browsefile' data-idx='"+(i+1).toString()+"'><img src='"+thumbplaceholderimg+(i+1).toString()+".jpg' OnError='this.src="+thumbHolder+"' /><br/><label>" +item.File + "</label></div>");
...
function saveFilesInFolder(folder) {
	$("#filesInFolder").html($("<img>").attr("src",'./images/loading.gif'))
	$.ajax({
		url: "services.php?type=save&folder="+folder,
        dataType: 'json'
	}).done(function( data ) {
		alert('Se copiaron los archivos.')
		$("#filesInFolder").html('');
		$.each(data, 
			function(i, item) {
				$("#filesInFolder").append("<div class='browsefile' data-idx='"+(i+1).toString()+"'><img src='"+thumbplaceholderimg,(i+1).toString()+"' /><br/><label>" +item.File + "</label></div>");
